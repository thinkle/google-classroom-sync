<script lang="ts">
    ...
    for (let postedGrade of postedGrades) {
      ...
      if (googleGrade.lastGradeChangeTimestamp <= postedGrade.timestamp) {
        console.log(
          "Grade",
          googleGrade,
          "was already posted at",
          postedGrade.timestamp
        );
        ...
        console.log(
          ...
          "was posted at",
          postedGrade.timestamp,
          "but has been updated since then."
        );
  ...
  function shouldPost(g) {
    return !g.alreadyPosted && g.aspenStudent && getGrade(g) != null;
  }
  ...
  async function pushGradesToAspen() {
    ...
    GoogleAppsScript.logGrades(
      googleAssignment.id,
      nonErrorResults.map(({ grade, result }) => ({
...
<Button
  primary={fetchingGrades == NOT_FETCHING}
  disabled={fetchingGrades != NOT_FETCHING && fetchingGrades != DONE_FETCHING}
  on:click={() => getGrades()}
>
  <div slot="icon" class="google-icon"></div>
  Fetch grades
</Button>
<FormItem>
...
{#if grades}
  ...
  {@const readyGrades = grades.filter(
    (g) => g.aspenStudent && getGrade(g) != null && !g.alreadyPosted
  )}
  <div class="grades">
    <Button
      on:click={() => pushGradesToAspen()}
...
{#if results && results.length}
  <h3 class="text-w-icon"><div class="aspen-icon">Posting Results:</div></h3>
  <ul>
